...
const PacScriptsTab = () => {
    ...
    const validatePacScript = (data, excludeId = null) => {
        ...
        const isDuplicateName = pacScripts.some(
            (script) => script.name.toLowerCase() === trimmedName.toLowerCase() && script.id !== excludeId
        );
    ...
    const saveEdit = async () => {
        ...
        try {
            ...
            const updatedScripts = pacScripts.map((script) => (script.id === editingScriptId ? updatedScript : script));
            setPacScripts(updatedScripts);
            setEditingScriptId(null);
            setEditFormData({ name: '', inputType: 'url', url: '', content: '', enabled: true });
            setOriginalEditUrl('');
    ...
    const reloadScript = async (script) => {
        ...
        try {
            ...
            await indexedDBStorage.updatePacScript(updatedScript);
            const updatedScripts = pacScripts.map((s) => (s.id === script.id ? updatedScript : s));
            setPacScripts(updatedScripts);
        ...
        <div className="space-y-6 min-h-[250px]">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-amber-700 mb-2">{messages.proxyControlWarningDescription}</p>
                            <p className="text-sm text-amber-600 font-medium">{messages.proxyControlWarningAction}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">{messages.pacScriptsTitle}</h3>
                </div>
                    ...
                    <div className="space-y-2">
                        {pacScripts.map((script) => (
                            <div
                                ...
                                onKeyDown={(e) => {
                                    if ((e.key === 'Enter' || e.key === ' ') && editingScriptId !== script.id) {
                                        e.preventDefault();
                                        startEdit(script);
                                    }
                                }}
                                role="button"
                                tabIndex={editingScriptId === script.id ? -1 : 0}
                            >
                                        ...
                                        {editingScriptId === script.id ? (
                                            <div className="space-y-3">
                                                <input
                                                    ...
                                                    className="w-full px-2 py-1 border border-slate-300 focus:border-slate-500 focus:ring-slate-500 rounded text-sm focus:outline-none focus:ring-1 break-words"
                                            ...
                                            <div className="flex items-center gap-2 min-w-0">
                                                <div
                                                    className="text-sm text-gray-900 py-1 truncate min-w-0 max-w-[270px]"
                                                    title={script.name}
                                                >
                                                    {script.name}
                                                </div>
                                                ...
                                                >
                                                    {script.sourceType === 'url'
                                                        ? messages.pacScriptTypeUrl
                                                        : messages.pacScriptTypePlain}
                                                </span>
                                        ...
                                        {editingScriptId === script.id ? (
                                            ...
                                            <>
                                                ...
                                                {script.sourceType === 'url' && (
                                                    ...
                                                    >
                                                        <ArrowPathIcon
                                                            className={`w-4 h-4 ${reloadingScript === script.id ? '' : ''}`}
                                                        />
                                                    </button>
                                                )}
                                                <button
                                                    ...
                                                    className="p-1 hover:bg-gray-100 rounded text-gray-500 hover:text-red-600 cursor-pointer"
                                                    title="Delete"
                                                >
                ...
                {(showForm || pacScripts.length === 0) && (
                    <div className="mt-4 p-4 bg-gray-50 rounded-md">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label htmlFor={scriptNameId} className="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <div className="flex gap-4">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setFormData({ ...formData, inputType: e.target.value, content: '' })
                                            }
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setFormData({ ...formData, inputType: e.target.value, url: '' })
                                            }
                                            className="text-slate-600 focus:ring-slate-500"
                                        />
            ...
            <Dialog
                ...
                onClose={() => setDeleteDialog({ isOpen: false, scriptId: null, scriptName: '' })}
            >
