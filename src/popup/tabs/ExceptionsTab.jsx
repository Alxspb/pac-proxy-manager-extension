...
import { CheckIcon, ChevronDownIcon, CogIcon, DocumentArrowUpIcon, XMarkIcon } from '@heroicons/react/24/outline';
        ...
        <div className="space-y-6 min-h-[250px]">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">{messages.exceptionsTitle}</h3>
                </div>

                {proxies.length === 0 && (
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-md mb-4">
                        <p className="text-sm text-amber-800">⚠️ {messages.exceptionsRequireProxies}</p>
                    </div>
                )}

                <div className="space-y-6">
                    <div>
                        <label htmlFor={domainInputId} className="block text-sm font-semibold text-gray-900 mb-2">
                                ...
                                <RadioGroup.Option value="no" disabled={!domain.trim()}>
                                    {({ checked, disabled }) => (
                                        ...
                                        >
                                            <XMarkIcon
                                                className={`w-4 h-4 flex-shrink-0 ${
                                                    disabled ? 'text-gray-400' : checked ? 'text-white' : 'text-red-500'
                                                }`}
                                            />
                                            <RadioGroup.Label
                                                className={`text-sm font-medium ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                        ...
                        {({ open }) => (
                            <>
                                <DisclosureButton className="flex w-full justify-between rounded-lg bg-slate-100 px-4 py-2 text-left text-sm font-medium text-slate-600 hover:bg-slate-100 focus:outline-none focus-visible:ring focus-visible:ring-slate-500 focus-visible:ring-opacity-75">
                                    ...
                                    <ChevronDownIcon
                                        className={`w-5 h-5 text-slate-500 ${open ? 'rotate-180 transform' : ''}`}
                                    />
                                </DisclosureButton>
                                <DisclosurePanel className="px-4 pt-4 pb-2 text-sm border border-gray-200 rounded-b-lg space-y-4">
                                    <div>
                                        ...
                                        <button
                                            ...
                                            disabled={
                                                selectedBulkOption === 0
                                                    ? !bulkImportYesText.trim()
                                                    : !bulkImportNoText.trim()
                                            }
